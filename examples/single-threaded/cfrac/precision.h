...
#define pSkip           ((precision *TRUSTED) -1)
...
#define pdiv(u,v)	(pdivmod(u,v, pSkip, pNull))
#define pmod(u,v)	(pdivmod(u,v, pNull, pSkip))
#define pdivr(u,v,r)	(pdivmod(u,v, pSkip, r))
#define pmodq(u,v,q)	(pdivmod(u,v, q, pSkip))
...
static inline precision pnew(precision u) hs_nofree { (* (prefc *) u)++; return u; }
#define pdestroy(u) if (1) { precision __tmp = (u); u = 0; actual_pdestroy(__tmp); } else 0
static inline void      actual_pdestroy(precision u) {
   if (u != pUndef && --(*(prefc *) u) == 0) pfree(u);
}
static inline precision pparmq(precision u) hs_nofree { 
   if (u != pUndef) (* (prefc *) u)++; return u;
}
static inline precision presult(precision u) hs_nofree {
   if (u != pUndef) --(*(prefc *) u); return u;
}
...
extern	precision	atop(char *NTS);	/* ascii to precision */
extern	char		*NTS ptoa(precision);	/* precision to ascii */
...
extern	precision	errorp(int errnum, char *NTS routine, char *NTS message) hs_nofree;
