...
extern unsigned short (COUNT(primesize) primes)[];
...
typedef struct SolnStruc {
   ...
   BitVector COUNT(BSIZEFOR(m)) e;/* bit vector of factorbase powers mod 2 */
   unsigned m;		/* bit vector size - 1 */
} Soln;

typedef Soln *SolnPtr;

void setBit(_bv, bno, value)
   register BitVector COUNT(BSIZEFOR(bno)) _bv;
...
BitVector COUNT(BSIZEFOR(size - 1)) newBitVector(value, size)
   register solnvec COUNT(size) value;
   unsigned size;
{
...
void printSoln(stream, prefix, suffix, _pm, m, p, t, _e)
   FILE *stream;
   char *NTS prefix, *NTS suffix;
   register unsigned *COUNT(m) _pm, m;
   precision p, t;
   register solnvec COUNT(m + 1) _e;
{
   solnvec e = _e;
   unsigned *pm = _pm;
...
void freeEas(eas, aborts)
   EasPtr COUNT(aborts + 1) eas;
   unsigned aborts;
{ 
 ...
 */
pfactors pfactorbase(n, k, m, aborts)
   precision n;
   unsigned k;
   unsigned m, aborts;
{
 ...
 */
EasPtr COUNT(aborts + 1) getEas(n, k, pm, m, aborts)
   precision n;
   unsigned k, *COUNT(m) pm, m, aborts;
{
   ...
   eas = (EasEntry *) malloc((aborts+1) * sizeof (EasEntry));
   if (eas == 0) return eas;
 ...
 */
int pfactorQ(f, t, _pm, _e, m, _eas, aborts)
   precision *f;
   precision t;
   register unsigned *COUNT(m) _pm;
   register solnvec  COUNT(m) _e;
   register unsigned m;
   EasPtr COUNT(aborts + 1) _eas;
{
   EasPtr eas = _eas;
   solnvec e = _e;
   unsigned *pm = _pm;
   ...
   unsigned *maxCount;
{
   ...
   ppm = pfactorbase(n, k, m, aborts); 		/* m may have been reduced */
   pm = ppm.primes;
   ppm.primes = NULL;
   m = ppm.len;
...
F2:
	 ...
	 e[0] = s;
	 i = pfactorQ(&t, Qn, pm, &e[1], m, eas, aborts);  	/* E3: Factor Qn */
	 if (i < -1) cfracNabort++;
	 /* 
