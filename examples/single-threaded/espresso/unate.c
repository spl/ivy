...
static int abs_select_restricted(pset_family A, pset rst);
              			...
              			/* disposes of A */
{
	 ...
	 */
	rst = set_new(A->sf_size);
	min_set_ord = A->sf_size + 1;
	foreachi_set(A, i, p) {
	    if (ESIZE(p) < min_set_ord) {
		set_copy(rst, p);
		min_set_ord = ESIZE(p);
	    } else if (ESIZE(p) == min_set_ord) {
		set_or(rst, rst, p);
	    }
	}
	...
	/* Check for "essential" columns */
	    ...
	    Abar = unate_complement(abs_covered_many(A, rst));
	    sf_free(A);
	...
	/* else, recur as usual */
	    ...
	    max_i = abs_select_restricted(A, rst);
...
pset_family exact_minimum_cover(pset_family T)
{
    ...
    pset_family temp;
    long start = ptime();
    struct localstack stack[32];                /* 32 suffices for 2 ** 32 cubes ! */
 ...
 *  also belongs to the set "rst"; weight each column of a set as
 *  1 / (set_ord(p) - 1).
 */
static int
abs_select_restricted(pset_family A, pset rst)
{
    register int i, best_var, best_count, *count;

    /* Sum the elements in these columns */
    count = sf_count_restricted(A, rst);
