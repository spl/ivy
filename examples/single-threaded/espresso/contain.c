...
int rm_equal(pset *A1, int (*compare) (/* ??? */))
                                /* updated in place */
                                ...
                                /* updated in place */
{
	...
	if (ESIZE(a) != last_size)
	    last_size = ESIZE(a), pcheck = pdest;
	for(pb = A1; pb != pcheck; ) {
	    b = *pb++;
...
int rm2_equal(register pset *A1, register pset *B1, pset *E1, int (*compare) (/* ??? */))
                                        /* updated in place */
                                ...
                                /* unchanged */
{
	...
	for(pb = B1; (b = *pb++) != NULL && ESIZE(b) > ESIZE(a); ) {
	    INLINEsetp_implies(a, b, /* when_false => */ continue);
