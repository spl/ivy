...
struct stage {
    ...
    char *SLOCKED(mut) COUNT(size) data;
    unsigned int SLOCKED(mut) size;
    int func;
    int SLOCKED(mut) last;
};

unsigned int do_work(char *COUNT(size) data, unsigned int size, int func);
...
void *thr_func(struct stage *S)
{
    ...
    int last = 0;
    unsigned int ret;
    char *COUNT(size) data = NULL, *COUNT(size) nextdata = NULL;
    ...
    while (!last) {
        ...
        S->data = NULL;
        S->size = 0;
	/* Data for the next stage is the same as this stage, except at the last
	   stage. This makes it legal to free data in the last stage. */
	if (S->next)
	  nextdata = data;
	else
	  nextdata = NULL;
...
FILE *inFile;
FILE *outFile;
static unsigned int do_work(char *COUNT(size) data, unsigned int size, int func)
{
    unsigned int ret;
    int i;
