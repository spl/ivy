...
int ftw(const char *NTS, int (*)(const char *NTS, const struct stat *, int), int);

extern char (NTS version)[];

char *NTS argv0 = "pfscan";


int max_depth = 64;

unsigned char *NTS rstr = NULL;
int rlen = 0;
...
int SLOCKED(&aworker_lock) aworkers = 0;
pthread_mutex_t SRACY aworker_lock;
pthread_cond_t  SRACY aworker_cv;

int line_f  = 0;
int maxlen  = 64;
int ignore_case = 0;

int SLOCKED(&matches_lock) n_matches = 0;
int n_files = 0;
size_t n_bytes = 0;

pthread_mutex_t SRACY matches_lock;

PQUEUE pqb;

pthread_mutex_t SRACY print_lock;
...
int
get_char_code(unsigned char *NTS *cp,
	      int base)
{
    int val = 0;
    int len = 0;
...
int
is_text(unsigned char *COUNT(slen) _cp, int slen)
{
...
size_t
print_output(unsigned char *COUNT(slen) _str,
	     size_t slen)
{
...
int
matchfun(unsigned char *NT COUNT(len) buf,
	 size_t len,
	 size_t pos,
	 void *misc)
{
    char *pathname = (char *NTS) TC(misc);
...
scan_file(char *NTS pathname)
{
	...
	if (verbose)
	{
	    ...
	    fprintf(stderr, "%s : ERR : stat() failed: %s\n", pathname, strerror(errno));
	...
	if (verbose)
	{
	    ...
	    fprintf(stderr, "%s : Skipping (not a regular file)\n", pathname);
    ...
    buf = (unsigned char *NT COUNT(len)) TC(mmap(NULL, len, PROT_READ, MAP_PRIVATE|MAP_NORESERVE, fd, 0));
    if (buf == (void *SNT)TC(-1)/*MAP_FAILED*/)
    {
	if (verbose)
	{
	    pthread_mutex_lock(&print_lock);
	...
	code = bm_search(&bmb, buf, len, matchfun, TC(pathname));
    }
    else
    {
	pthread_mutex_lock(&print_lock);
...
int
foreach_path(const char *NTS path,
...
worker(void *arg)
{
    char *NTS *path = malloc(sizeof(char *));

    while (pqueue_get(&pqb, /*(void **)*/ TC(path)) == 1)
    {
	scan_file(*path);
	HS_ZFREE(*path);
    }
