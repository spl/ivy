...
static void
preBmBc(unsigned char *COUNT(m-1) x,
	int m,
	int (COUNT(BM_ASIZE) bmBc)[])
{
    int i;
...
static void
suffixes(unsigned char *COUNT(m) x,
	 int m,
	 int *COUNT(m) suff)
{
    int f, g, i;
...
static int
preBmGs(unsigned char *COUNT(m) x, int m, int (COUNT(m) bmGs)[])
{
    int *suff, i, j;
