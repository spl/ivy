...
INLINE static void CPtrArith(const volatile void* lo, const volatile void* hi,
			     ...
			     __LOCATION__FORMALS) {
  if (e >= 0) {
    DEPUTY_ASSERT_TEXT((unsigned)e <= (hi - p) / sz, texthi, "upper bound check");
  } else {
    DEPUTY_ASSERT_TEXT((unsigned)-e <= (p - lo) / sz, textlo, "lower bound check");
  }
}
...
INLINE static void CPtrArithAccess(const volatile void* lo,
				   ...
				   __LOCATION__FORMALS) {
  if (e >= 0) {
    DEPUTY_ASSERT_TEXT((unsigned)e + 1 <= (hi - p) / sz, texthi, "upper bound check");
  } else {
    DEPUTY_ASSERT_TEXT((unsigned)-e <= (p - lo) / sz, textlo, "lower bound check");
  }
}
